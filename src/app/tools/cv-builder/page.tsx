...
export default function CVBuilderPage() {
  ...
  const handlePrint = async () => {
    ...
    if (!isPro) {
      ...
      if (userSnap.exists()) {
        const lastExport = userSnap.data().lastPdfExportDate;
        if (lastExport) {
          const daysSinceLastExport = (Date.now() - lastExport) / (1000 * 60 * 60 * 24);
          if (daysSinceLastExport < 7) {
            toast.error('QUOTA_EXCEEDED', 'Guest users can only export to PDF once a week. Please upgrade to Pro.');
            return;
          }
        }
      }
      await setDoc(userRef, { lastPdfExportDate: Date.now() }, { merge: true });
    }
    ...
    try {
      ...
      const opt: any = {
        margin:       0,
        filename:     `${watchedData.personalInfo.fullName.replace(/\s+/g, '_')}_CV.pdf`,
